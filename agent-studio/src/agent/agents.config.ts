...
export const DEFAULT_AGENTS: AgentConfig[] = [
  ...
  },
  {
    ...
    prompt: [
      ...
      '레퍼런스 이미지 경로를 받으면 먼저 Read로 레퍼런스를 보고, 구현 뒤 `--node=<대상 노드>`로 찍은 스크린샷과 나란히 비교해 색·비율·간판 위치·디테일의 차이를 목록으로 적고 고친다. 비교·수정은 최대 3번 반복하고, 코드 그리기로 따라갈 수 없는 부분은 보고에 적는다. 레퍼런스 파일은 참고용이며 게임 코드에서 불러오지 않는다.',
...
## 레퍼런스 기반 시각 작업
외형(건물·차·HUD·이펙트 등)을 "더 멋지게/이런 느낌으로" 바꾸는 요청이거나 사용자가 레퍼런스를 원하면 이 순서로 한다.
작업 폴더 규칙상 이미지를 게임 에셋으로 넣을 수 없는 프로젝트(예: 전부 코드로 그리는 게임)에서는 레퍼런스가 "참고용 그림"일 뿐이며 게임에 넣지 않는다.
1. 대상을 정한다. 스크린샷 도구가 있으면(예: tools/screenshot.gd) --list 로 대상 노드 이름을 찾고, 지금 모습을 --node 로 찍어 둔다.
2. Codex에게 mode=image로 레퍼런스를 만든다. savePath는 docs/refs/<대상>-<번호>.png. 프롬프트(영어)에는 게임 시점과 표현 한계를 반드시 넣는다
   (예: top-down orthographic view seen from directly above, flat 2D shapes and solid colors, no perspective, readable at small size, game art reference sheet).
3. confirm_reference 도구로 사용자에게 보여 주고 승인받는다. 승인 전에는 구현하지 않는다. 거절되면 사용자의 추가 지시를 반영해 프롬프트를 고치고 다른 번호로 다시 만든 뒤 다시 확인받는다 (최대 3번, 그 뒤엔 사용자에게 방향을 묻는다).
4. 승인되면 그리기 담당 서브에이전트에게 맡긴다. 레퍼런스 경로, 대상 노드 이름, 지금 모습 스크린샷 경로, "레퍼런스와 스크린샷을 나란히 비교해 차이를 줄이고, 비교·수정을 최대 3번 반복하라"를 전달한다.
