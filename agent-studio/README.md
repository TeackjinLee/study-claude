...
4. **직접 대화**: 명령창에 `/codex <메시지>`(대화·작업 요청, 파일 수정 가능; 첨부한 이미지는 Codex가 직접 보고, 파일은 경로를 받아 읽습니다), `/codex review <메시지>`, `/codex implement <메시지>`, `/codex image [>저장경로] <프롬프트>`(이미지 생성), `/codex @<에이전트> <메시지>`, `/codex reset`. 입력창 대상이 Codex일 때(`/codex ` 뒤에서) `/`를 치면 Codex 하위 명령 메뉴가 뜹니다: `/codex /model [이름]`, `/codex /status`(로그인·모델·구독 한도), `/codex /reset`, `/codex /help`, `/codex /review …` 등. 실행 중에는 쓸 수 없습니다.
...
6. **권한**: 에이전트 편집기에서 Codex 에이전트마다 `Edit`(기존 파일 수정) / `Write`(새 파일 생성) / `Bash`(명령 실행) 권한을 켭니다. 모두 끄면 어떤 모드든 읽기 전용 샌드박스, 하나라도 켜면 `review`를 뺀 모드(`discuss`/`implement`/`image`)에서 `workspace-write` 샌드박스로 돌고 세부 제한(예: 생성 금지)은 지시문으로 전달됩니다. 쓰기 호출은 권한 모드가 `default`면 승인 배너가 뜨고 `acceptEdits`면 자동입니다. `/codex`로 직접 말할 때는 승인 없이 바로 고칩니다. 기본 Codex 에이전트는 세 권한이 모두 켜져 있습니다.
